...
def main():
    ...
    with col2:
        ...
        with m1:
            st.markdown("### 📈 风机性能曲线")
            st.info("专业的风机性能曲线图谱工具。支持从离线计算数据进行插值，生成包含各类工程边界（如喘振线、做功效率等）的精美可导出图谱。")
            
        with m2:
            st.markdown("### ⚖️ 动平衡工作站")
            st.success("基于 ISO 1940 标准的参数测算与判定系统。支持许用不平衡力矩评估及各类实体制品钻孔去重的深度计算指导。")
            
        with m3:
            st.markdown("### 💨 气体计算器")
            st.warning("通用热力学气体参数转换引擎。提供多工质纯气体及标准空气压缩绝热功率计算，以及状态体积、质量流量物理换算。")

        st.markdown("---")
        st.markdown(
            """
            <div style='text-align: center; color: #64748b; font-size: 0.9rem; margin-top: 50px;'>
                👈 请从 <b>左侧导航栏</b> (Sidebar) 点击进入相应的计算模块
                <br><br>
